<div>
    <div class="flex flex-col px-8 mx-auto max-w-7xl xl:px-5 py-12">
        ...
        <div class="flex justify-between py-2 flex-wrap items-center">
            <div class="justify-end md:justify-start items-center gap-2 inline-flex flex-wrap mt-4" style="height: fit-content;">
                <div class="relative w-44">
                    ...
                    <input name="dates" type="text" wire:change="handleChangeSearchDate" id="visit_daterange" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full pl-10 p-2.5" placeholder="Select Dates">
                </div>
                @if (auth()->user()->currentTeam)
                <div class="relative w-44">
                    <select id="countries" wire:model="filterProvider" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
            ...
            <div class="flex gap-2 flex-wrap justify-end mt-4 ml-auto">
                ...
                <button data-modal-target="uploadVisitModal" data-modal-toggle="uploadVisitModal" class="w-44 px-2 py-2 bg-indigo-600 rounded-md border border-gray-300 justify-center items-center inline-flex">
                    <div class="justify-start items-start gap-2 flex">
                        <div class="w-5 h-5 relative">
            ...
            <table class="w-full text-sm text-left text-gray-500">
                    ...
                    <tr>
                        <th scope="col" class="p-4" style="width: 48px;">
                            <div class="flex items-center">
                                <input id="checkbox-all-search" type="checkbox" wire:change="handleSelectAll($event.target.checked)" class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 focus:ring-2">
                ...
                <tbody>
                    ...
                    <tr class="bg-white border-b hover:bg-gray-50">
                        <td class="w-4 p-4">
                            <div class="flex items-center">
                                <input id="checkbox-table-search-1" type="checkbox" wire:change="handleSelectRow({{ $visit->id }})" {{ in_array($visit->id, $selected_rows) ? 'checked' : '' }} class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 focus:ring-2">
                            ...
                            <a href="{{ route('wave.visits.fill_out', $visit->id) }}" class="font-medium text-blue-600 hover:underline flex justify-center">
                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 26 27.5" x="0px" class="w-5 h-5" y="0px">
                                    ...
                                    <g data-name="Layer 2">
                                        ...
                                        by Mohamed Salah Hajji</text><text x="0" y="42" fill="#000000" font-size="5px" font-weight="bold" font-family="'Helvetica Neue', Helvetica, Arial-Unicode, Arial, Sans-serif">from
                                        the Noun Project</text>
                                </svg>
                            </a>
                        </td>
                        <td class="px-2 py-4">
                            <div class="cursor-pointer" data-dropdown-toggle="edit_dropdown{{ $visit->id }}">
                                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16" fill="none">
                                    <path fill-rule="evenodd" clip-rule="evenodd" d="M3.25023 10.26L5.72023 12.73L12.4102 6.03999L9.95023 3.55999L3.25023 10.26ZM0.990234 14.99L4.85023 13.6L2.39023 11.16L0.990234 14.99ZM13.2402 0.98999C12.7602 0.98999 12.3202 1.18999 12.0002 1.49999L10.5602 2.93999L13.0302 5.40999L14.4702 3.96999C14.7902 3.64999 14.9802 3.21999 14.9802 2.72999C14.9902 1.77999 14.2102 0.98999 13.2402 0.98999Z" fill="#101840" />
                                </svg>
                                <div id="edit_dropdown{{ $visit->id }}" class="z-10 hidden bg-white divide-y divide-gray-100 rounded-lg shadow w-48 dark:bg-gray-700">
                                    <ul class="py-2 text-xs text-gray-700 dark:text-gray-200" aria-labelledby="dropdownDefaultButton">
                                        <li>
                                            <button onclick="openEditVisitModal({{ $visit->id }})" class="block px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white w-full">Edit
                                                Visit</button>
                                        </li>
                                        <li>
                                            <button wire:click="handleOpenQuickNoteModal({{ $visit->id }})" class="block px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white w-full">Complete
                                                Visit Electronically</button>
                                        </li>
                                    </ul>
                                </div>
                            </div>
        ...
        @if ($generate_note_id)
        <!-- Main modal -->
        <div id="quickNoteModal" tabindex="-1" aria-hidden="true" class="fixed top-0 left-0 right-0 z-50 w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-[calc(100%-1rem)] max-h-full justify-center items-center flex">
            <div class="relative w-full max-w-2xl max-h-full">
                ...
                <div class="relative bg-white rounded-lg shadow">
                    ...
                    <div class="flex items-start justify-between p-4 border-b rounded-t">
                        ...
                        <button type="button" wire:click="closeQuickNoteModal" class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ml-auto inline-flex justify-center items-center">
                    ...
                    <form wire:submit.prevent="generate_note">
                        ...
                        <div class="p-6 space-y-6">
                            @if (!$note_generating)
                            <textarea id="prompt" name="prompt" wire:model="prompt" rows="6" placeholder="Briefly describe your encounter, almost like you would when presenting on rounds and let AI generate your note for you" class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Write your thoughts here..."></textarea>
                            @else
                        ...
                        <div class="flex items-center justify-end p-6 space-x-2 border-t border-gray-200 rounded-b">
                            ...
                            @endif
                            <div wire:click="closeQuickNoteModal" type="button" class="text-gray-500 cursor-pointer bg-white hover:bg-gray-100 focus:ring-4 focus:outline-none focus:ring-blue-300 rounded-lg border border-gray-200 text-sm font-medium px-5 py-2.5 hover:text-gray-900 focus:z-10">
                                Close
                            </div>
                        </div>
                    </form>
        ...
        <div id="printVisitsPreviewModal" data-modal-backdrop="static" tabindex="-1" aria-hidden="true" class="fixed top-0 left-0 right-0 z-50 w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 max-h-full {{ $showPreviewModal ? '' : 'hidden' }}">
            <div class="relative w-full max-w-2xl max-h-full z-20" style="{{ $showPreviewModal ? 'top: 50%; left: 50%; transform: translate(-50%, -50%)' : '' }}">
                ...
                <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                    ...
                    <div class="flex items-start justify-between p-4 border-b rounded-t dark:border-gray-600">
                        ...
                        <button wire:click="closePrintVisits" type="button" class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ml-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white" data-modal-hide="printVisitsPreviewModal">
                            <svg class="w-3 h-3" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 14 14">
                                <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m1 1 6 6m0 0 6 6M7 7l6-6M7 7l-6 6" />
                            </svg>
                            <span class="sr-only">Close modal</span>
                    ...
                    <div class="flex justify-end items-center p-6 space-x-2 border-t border-gray-200 rounded-b dark:border-gray-600">
                        <button wire:click="downloadZipFiles" type="button" class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">Download
                            All</button>
                        <button wire:click="closePrintVisits" type="button" class="text-gray-500 bg-white hover:bg-gray-100 focus:ring-4 focus:outline-none focus:ring-blue-300 rounded-lg border border-gray-200 text-sm font-medium px-5 py-2.5 hover:text-gray-900 focus:z-10 dark:bg-gray-700 dark:text-gray-300 dark:border-gray-500 dark:hover:text-white dark:hover:bg-gray-600 dark:focus:ring-gray-600">Close</button>
                    </div>
                </div>
            </div>
