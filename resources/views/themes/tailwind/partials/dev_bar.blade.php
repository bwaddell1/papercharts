            ...
            <div class="w-full h-full border-t border-blue-500 bg-gradient-to-r from-wave-500 via-blue-500 to-purple-600" :class="{ 'overflow-hidden': open }">
                <div class="flex justify-between w-full h-full">
                    <div class="flex h-full">
                        ...
                        <div @click="open=true; url='/docs'; active='docs';" class="flex items-center justify-center h-full text-xs leading-none text-blue-100 border-l border-blue-500 cursor-pointer hover:bg-blue-600" :class="{ 'px-3': !open, 'px-5': open, 'bg-blue-600': active == 'docs' && open, 'bg-wave-500': active != 'docs' }">
                            Documentation
                        </div>
                        @if(!auth()->guest() && auth()->user()->can('browse_admin'))
                            <div @click="open=true; url='/admin'; active='admin';" class="flex items-center justify-center h-full text-xs leading-none text-blue-100 border-l border-blue-500 cursor-pointer hover:bg-blue-600" :class="{ 'px-3': !open, 'px-5': open, 'bg-blue-600': active == 'admin' && open, 'bg-wave-500': active != 'admin' }">
                                Admin
                            </div>
                        @endif
                    </div>
